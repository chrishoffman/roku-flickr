...
Function InitFlickr() As Object
	flickr = {
        oauth_prefix: "https://www.flickr.com/services/oauth",
        link_prefix: getLinkWebsite(),
        api_prefix: "https://api.flickr.com/services/rest",
...
Function ExecServerAPI(method, param_list=[] As Object) As Object
    oa = Oauth()

	apiurlstr="https://api.flickr.com/services/rest/?method="+method
	...
	'print apiurlstr
	http=NewHttp(apiurlstr)

    'Send Oauth signed or with just api_key
    if oa.authsecret <> "" then
        oa.sign(http,true)
    else
        http.addParam("api_key",getApiKey())
    end if
...
Sub AddNextPhotoToSlideShow(ss, photolist)
	...
	if photolist.IsNext() then
		...
		if s<>invalid then
            ...
            photoUrl = strReplace(s@source, "https://", "http://")
            print photoUrl
			photo.Info.url=photoUrl
...
Function GetUserIDByURL(userurl as String) As String
  ...
  m.http.SetUrl("https://api.flickr.com/services/rest/?method=flickr.urls.lookupUser&url=//flickr.com/photos/"+userurl+"/"+"&api_key=1beba5866bc14edec5bff26091cecc2c")
...
Function GetInterestingnessPhotoList(page=1 As Integer, per_page=100 As Integer) As Object
    ...
    rsp=m.ExecServerAPI("flickr.interestingness.getList",["page="+itostr(page),"per_page="+itostr(per_page)])
	if rsp@stat="ok" then
...
Sub DisplayMyPhotoStream()
    if doRegistration() then  ' in regScreen.brs
...
Function GetGroupPhotoPoolByURL(http as Object, groupurl as String) As Object
  ...
  http.SetUrl("https://api.flickr.com/services/rest/?method=flickr.urls.lookupGroup&url=//flickr.com/groups/"+groupurl+"/"+"&api_key=1beba5866bc14edec5bff26091cecc2c")
  ...
  http.SetUrl("https://api.flickr.com/services/rest/?method=flickr.groups.pools.getPhotos&group_id="+group_id+"&api_key=1beba5866bc14edec5bff26091cecc2c")
  xml=http.GetToString()
...
Function GetTaggedPhotoList(tags As String, page=1 As Integer, per_page=50 As Integer) As Object
	rsp=m.ExecServerAPI("flickr.photos.search", ["tags="+m.http.Escape(tags), "sort=interestingness-desc", "page="+itostr(page), "per_page="+itostr(per_page)])
	if rsp@stat="ok" then
		return newPhotoListFromXML(m.http, rsp.photos.photo)
	else
		return []
...
'	</tags>
'	<urls>
'		<url type="photopage">https://www.flickr.com/photos/bees/2733/</url>
'	</urls>
'</photo>
